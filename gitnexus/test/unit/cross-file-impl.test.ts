...
describe('runCrossFileBindingPropagation', () => {
  ...
  it('stops processing when time deadline is exceeded', async () => {
    const graph = createKnowledgeGraph();
    const ctx = createResolutionContext();
    ...
    // Create 50 downstream importers
    ctx.importMap.set('upstream.ts', new Set());
    for (let i = 0; i < 50; i++) {
    ...
    // Set a very short timeout via env var. The processCalls mock below
    // advances Date.now past this deadline after the first invocation.
    const origEnv = process.env.GITNEXUS_CROSS_FILE_TIMEOUT_MS;
    process.env.GITNEXUS_CROSS_FILE_TIMEOUT_MS = '50';

    let callCount = 0;
    processCallsMock.mockImplementation(async () => {
      callCount++;
      // After the first call, busy-wait past the 50ms deadline so the
      // time check before the next candidate triggers.
      if (callCount === 1) {
    ...
    try {
      ...
      // Should have processed at least 1 file (the first one goes through
      // before the deadline check), but NOT all 50.
      expect(result).toBeGreaterThan(0);
      expect(result).toBeLessThan(50);
      expect(processCallsMock.mock.calls.length).toBeLessThan(50);
    } finally {
      if (origEnv === undefined) {
        delete process.env.GITNEXUS_CROSS_FILE_TIMEOUT_MS;
      } else {
        process.env.GITNEXUS_CROSS_FILE_TIMEOUT_MS = origEnv;
      }
      processCallsMock.mockImplementation(async () => []);
    }
  });
